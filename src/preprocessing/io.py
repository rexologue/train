...
def resolve_split_paths(config: Config, splits: list[str]) -> list[tuple[str, Path]]:
    """Resolve configured split parquet paths, skipping optional missing test split."""

    resolved: list[tuple[str, Path]] = []
    for split in splits:
        raw_value = getattr(config.preprocessing.raw, f"{split}_path")

        if raw_value is None:
            if split == "test" and not config.preprocessing.raw.test_required:
                continue
...
def read_raw_dataframe(raw_path: Path) -> Any:
    """Read raw parquet and validate the authoritative `data` + `type` wrapper."""

    frame = pd.read_parquet(raw_path)
    columns = set(frame.columns)

    if "target" in columns:
        raise ParquetSchemaError(f"{raw_path}: raw parquet must not contain target; use type")

    if "data" not in columns or "type" not in columns:
        raise ParquetSchemaError(f"{raw_path}: raw parquet must contain data and type")
...
def dataframe_to_rows(frame: Any) -> list[dict[str, Any]]:
    ...
    for row_index, record in enumerate(frame.to_dict(orient="records")):
        ...
        loss_kind = record["type"]

        if loss_kind not in LOSS_KINDS:
            raise ParquetSchemaError(f"row {row_index}: unsupported type={loss_kind!r}")
        ...
        rows.append({"row_index": row_index, "loss_kind": loss_kind, "payload": payload})

    return rows


def read_rows(path: str | Path) -> list[dict[str, Any]]:
    """Read raw parquet into decoded payload rows with `loss_kind`."""
    ...
    for row in dataframe_to_rows(read_raw_dataframe(Path(path))):
        ...
        metadata["parquet_type_column"] = "type"
...
def load_pretokenized_split_results(config: Config, splits: list[str]) -> list[PretokSplitResult]:
...
def split_manifest_from_cache_manifest(manifest: dict[str, Any], split: str) -> dict[str, Any]:
    """Extract the per-split manifest shape returned by preprocessing callers."""

    splits = manifest.get("splits") if isinstance(manifest.get("splits"), dict) else {}
    pretokenized = manifest.get("pretokenized") if isinstance(manifest.get("pretokenized"), dict) else {}
    rejections = manifest.get("rejections") if isinstance(manifest.get("rejections"), dict) else {}
    ...
    return {
        "input_sha256": splits.get(split),
        "pretokenized_sha256": pretokenized.get(split),
        ...
        "rejected_counts": rejections.get(split) or {},
    }
